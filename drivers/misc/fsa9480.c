...
static int FSA_RESET_VALUE = 0;
	...
	|| defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC) || defined(CONFIG_MACH_NEVIS3G)
...
#elif defined(CONFIG_MACH_BAFFIN_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
		if ( board_hw_revision >= 2 )		
		{
			fsa9485_uart_sel_switch_en(1);
			path = AUTO;
...
static void fsa9480_i2c_gpio_init(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "FSA_RESET_VALUE : %d\n", FSA_RESET_VALUE);	
